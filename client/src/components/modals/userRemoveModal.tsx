...
const UserRemoveModal: React.FC = () => {
    ...
    const { userProfileAlertMessage, isUserRemoveProfile, closeUserRemoveProfile } = useUserProfileContexts();
    const { userAccountAlertMessage, isUserRemoveAccount, hideUserAccountRemoveModal } = useRemoveAccountModal();

    // here was define function for call destruct data of custom hooks
    const handleCloseModal = () => {
        // check condition
        if (isUserRemoveProfile) {
            closeUserRemoveProfile();
        }
        else if (isUserRemoveAccount) {
        ...
        <>
            ...
            <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                <div className="relative bg-white rounded-lg shadow-lg p-6 mb-12 md:mb-96 lg:mb-80 w-11/12 sm:w-96 max-w-lg">
                    ...
                    <button type="button" onClick={handleCloseModal} className="absolute top-3 right-3 text-gray-400 bg-transparent 
                    ...
                    <div className="p-4 md:p-5 text-center">
                        ...
                        <h3 className="mb-5 text-base sm:text-lg font-normal text-gray-500 dark:text-gray-400">
                            ...
                            {isUserRemoveAccount && userAccountAlertMessage}
                        </h3>

                        <div className="flex flex-col sm:flex-row justify-center sm:justify-end px-6 space-y-4 sm:space-y-0 sm:space-x-4">
                            <button type="button" className="px-4 py-2 bg-red-500 text-base md:text-sm text-white 
                            rounded hover:bg-red-600 w-full sm:w-auto">
                                Yes, I'am confirm
                            </button>
                            <button type="button" onClick={handleCloseModal} className="px-4 py-2 bg-blue-500 text-base md:text-sm text-white 

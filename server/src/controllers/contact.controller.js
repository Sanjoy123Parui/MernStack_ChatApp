  ...
  if (!userSignup) {
    ...
    if (!(contact_name && contact_phone)) {
      ...
      if (existContact) {
        ...
        let contactUser = await userSignupModel
          .findOne({ phone: contact_phone })
          .exec();

        let existContact = await userProfileModel
          .findOne({
        ...
        if (existContact.userSignup.phone !== contact_phone) {
          return next(
            errorHandler("Please required the valid phone number", 400)
          );
        } else {
...
const viewAllContact = asyncHandler(async (req, res, next) => {
  ...
  if (!userSignup) {
    ...
    if (cache.has("existUserprofile")) {
      ...
      existUserprofile = await userProfileModel
        .findOne({
          userSignup: userSignup,
        })
        .exec();
...
const searchContact = asyncHandler(async (req, res, next) => {
  ...
  if (!userSignup) {
    ...
    if (!(contact_phone || contact_name)) {
      return next(
        errorHandler("Please required correct contact phone or name ", 400)
      );
    } else {
      // here are declare query for fetch the particular contact search data from database
      let userSearchContact = await contactModel
        .find({
          userProfile,
          $or: [{ contact_phone }, { contact_name }],
        })
        .populate([
